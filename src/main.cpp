...
// Forward declaration of CUDA functions from utils_model.cu
...
int main(int argc, char* argv[]) {
    ...
    // Allocate memory for the normalized float image
    float* d_resized_and_normalized_image;
    int num_pixels = YOLO_INPUT_WIDTH * YOLO_INPUT_HEIGHT * 3; // Total number of pixels
    cudaMalloc(&d_resized_and_normalized_image, num_pixels * sizeof(float));

    // Convert resized image to float and normalize it (dividing by 255)
    convert_image_to_float(d_resized_image, d_resized_and_normalized_image, num_pixels);

    // Run YOLO object detection using the resized and normalized image
    std::vector<BoundingBox> faces = run_inference(engine, d_resized_and_normalized_image, num_pixels * sizeof(float));
    ...
    if (!d_kitty_emoji) {
        ...
        cudaFree(d_image);
        cudaFree(d_resized_image);
        cudaFree(d_resized_and_normalized_image);
        return 1;
    }

    // Allocate memory for the output image (in unsigned char)
    unsigned char* d_final_image;
    cudaMalloc(&d_final_image, num_pixels * sizeof(unsigned char));

    // Convert normalized image back to unsigned char for further processing
    convert_float_to_uchar(d_resized_and_normalized_image, d_final_image, num_pixels);

    // Replace each detected face with the kitty emoji
    for (const auto& face : faces) {
        replace_with_kitty(d_final_image, YOLO_INPUT_WIDTH, YOLO_INPUT_HEIGHT, face, d_kitty_emoji, kitty_width, kitty_height);
    }

    // Save the modified image back to disk
    if (!write_image_from_gpu(output_image, d_final_image, YOLO_INPUT_WIDTH, YOLO_INPUT_HEIGHT)) {
